...
namespace SC.DAL.EF
{
    internal class SupportCenterDbInitializer : DropCreateDatabaseIfModelChanges<SupportCenterDbContext>
    {
        protected override void Seed(SupportCenterDbContext context)
        {
            ...
            {
                AccountId = 1,
                Text = "Ik kan mij niet aanmelden op webmail",
            ...
            context.TicketResponses.Add(t1r1);

            TicketResponse t1r2 = new TicketResponse()
            {
                Ticket = t1,
                Text = "Account terug inorde en nieuw passwoord ingesteld",
                Date = new DateTime(2012, 9, 9, 13, 29, 11),
                IsClientResponse = false
            };
            context.TicketResponses.Add(t1r2);
